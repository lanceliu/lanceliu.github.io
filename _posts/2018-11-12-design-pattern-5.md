...
Visitor  VS Iterator
vistor 致力于不改变对象结构，可以动态添加功能
Iterator 致力于采取统一的方式去访问每个对象
...
而状态模式封装的是不同的状态，以达到状态切换行为随之发生改变的目的。这两种模式虽然都有变换的行为，但是两者的目标却是不同的

比如人在老中青三个阶段主要目标和行为是不一样的

- 对象作为参数
一些模式引入总是被用作参数的对象。一个Visitor对象是一个多态的Accept操作的参数，这个操作作用于该Visitor对象访问的对象。
